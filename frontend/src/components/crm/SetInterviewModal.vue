<template>
  ...
  >
    ...
    >
      <!-- Modal Header -->
      <div class="bg-primaryColor !font-medium text-white p-6">
        ...
        >
          ...
          <div class="flex justify-end gap-4 pt-6 border-t border-gray-200">
            ...
            <button
              ...
              class="btn btn-sm bg-primaryColor text-white font-thin hover:bg-primaryColor/80"
            >
              <span
                v-if="isSubmitting"
...
<script setup>
  ...
  const scheduleInterview = async () => {
    try {
      ...
      if (result.success) {
        ...
        showSuccess(
          'Interview scheduled successfully! Redirecting to Interview Schedule...'
        );
  ...
  const updateApplicationStatus = async (status) => {
    try {
      const response = await fetch(
        `/api/job-applications/${props.applicationId}/status`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
