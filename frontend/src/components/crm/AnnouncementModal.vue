<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 overflow-y-auto" style="z-index: 9999;">
    ...
    <!-- Modal -->
    <div class="flex min-h-full items-center justify-center p-2 sm:p-4 relative">
      ...
      >
        ...
        <div class="p-4 sm:p-6">
          ...
          <div
            v-if="announcement?.promo_details && announcement?.promo_position === 'above'"
          ...
          <template v-for="(item, index) in getContentOrder()" :key="item">
            ...
            <!-- Images - Single -->
            <div
              v-if="item === 'images' && getAnnouncementImages().length > 0 && announcement?.image_display_type === 'single'"
            ...
            <!-- Images - Carousel -->
            <div
              v-if="item === 'images' && getAnnouncementImages().length > 1 && announcement?.image_display_type === 'carousel'"
            ...
            >
              <div class="relative w-full">
                ...
                />
                <div v-if="getAnnouncementImages().length > 1" class="absolute inset-0 flex items-center justify-between p-2 sm:p-4">
                  <button
                  ...
                  <button
                    ...
                    class="btn btn-circle btn-sm sm:btn-md bg-black/50 text-white border-none hover:bg-black/70"
                    :disabled="currentImageIndex === getAnnouncementImages().length - 1"
                  >
                    <font-awesome-icon icon="fa-solid fa-chevron-right" class="w-4 h-4" />
                  </button>
                </div>
                <div v-if="getAnnouncementImages().length > 1" class="absolute bottom-2 sm:bottom-4 left-1/2 transform -translate-x-1/2 flex gap-1 sm:gap-2">
                  <div
                    v-for="(image, imgIndex) in getAnnouncementImages()"
                    :key="imgIndex"
                    class="w-2 h-2 sm:w-3 sm:h-3 rounded-full transition-all"
                    :class="currentImageIndex === imgIndex ? 'bg-white' : 'bg-white/50'"
                  ></div>
                </div>
              </div>
            </div>
            ...
            >
              <div class="relative w-full" style="padding-bottom: 56.25%;">
                <iframe
                  v-if="isYouTubeUrl(announcement.video_url) || isVimeoUrl(announcement.video_url)"
          ...
          <div class="space-y-4 sm:space-y-6">
            ...
            <div v-if="announcement?.action_link" class="pt-2">
              <a
                ...
                class="w-full text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-medium sm:font-semibold transition-all duration-300 flex items-center justify-center space-x-2 group shadow-lg hover:shadow-xl hover:-translate-y-1 text-sm sm:text-base hover:opacity-90"
...
<script setup>
  ...
  const getAnnouncementImages = () => {
    ...
    if (announcement.value.images) {
      try {
        const images = typeof announcement.value.images === 'string' 
  ...
  const startCarouselAutoNext = () => {
    ...
    if (currentAnnouncement && currentAnnouncement.image_display_type === 'carousel') {
      const images = getAnnouncementImages();
      if (images.length > 1) {
  ...
  const getContentOrder = () => {
    ...
    if (announcement.value.content_order) {
      try {
        const order = typeof announcement.value.content_order === 'string' 
...
<style scoped>
  ...
  * {
  ...
  /* Allow inline styles from HTML content */
  :deep(.prose div),
  :deep(.prose span) {
    /* Preserve inline styles from user HTML */
  }
</style>
