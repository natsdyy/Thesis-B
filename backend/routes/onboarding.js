...
router.post("/generate-token", async (req, res) => {
  try {
    ...
    if (app.status !== "hired") {
      return res.status(400).json({
        success: false,
        message: "Application must be in 'hired' status to generate onboarding link"
      });
    }
...
router.get("/verify/:token", async (req, res) => {
  try {
    ...
    if (decoded.resubmission && decoded.employee_id) {
      ...
      if (existingEmployee && (existingEmployee.onboarding_status === "rejected" || existingEmployee.onboarding_status === "pending")) {
        // Pre-fill with employee's existing data
        const department = existingEmployee.department || app.department || app.department_name || "";
        const roleName = existingEmployee.role_id ? 
          (await db("user_roles").where("role_id", existingEmployee.role_id).whereNull("deleted_at").first())?.role || 
        ...
        return res.json({
          ...
          data: {
            ...
            emergency_contact_address: existingEmployee.emergency_contact_address,
          },
          isResubmission: true,
    ...
    if (!decoded.resubmission) {
      ...
      if (existingEmployee && existingEmployee.onboarding_status === "approved") {
        return res.json({
          success: true,
...
router.post("/submit", async (req, res) => {
  try {
    ...
    console.log("Creating employee with data:", JSON.stringify(employeeWithStatus, null, 2));
  ...
  try {
    ...
    let query = db("employees as e")
      .leftJoin("branches as b", function() {
        this.on("e.branch_id", "=", "b.id")
          .andOnNull("b.deleted_at");
      })
      .leftJoin("user_roles as ur", function() {
        this.on("e.role_id", "=", "ur.role_id")
          .andOnNull("ur.deleted_at");
      })
    ...
    const formattedSubmissions = submissions.map(sub => ({
      ...
      full_name: `${sub.first_name || ''} ${sub.middle_name || ''} ${sub.last_name || ''}`.trim(),
  ...
  try {
    ...
    if (employee.email) {
      try {
        ...
        const passwordForEmail = defaultPassword || (employee.password ? employee.last_name : null);
      ...
      console.log(`ℹ️ Employee ${employee.email} already has an account - account activated`);
    }
...
router.post("/request-resubmission", authenticateToken, async (req, res) => {
  try {
    ...
    const EmailService = require("../services/emailService");
    await EmailService.sendResubmissionRequestEmail(
      employee.email,
      {
