<template>
  ...
  >
    ...
    >
      <!-- Modal Header -->
      <div class="bg-primaryColor text-white p-6">
        <div class="flex justify-between items-center">
          <div>
            <h2 class="text-2xl font-medium mb-2">Application Details</h2>
            <p class="text-green-100">
              {{ application?.full_name || 'Loading...' }}
            </p>
          </div>
          <button
            @click="closeModal"
        ...
        <div v-else-if="application" class="space-y-6">
          ...
          <div class="bg-gray-50 rounded-lg p-6">
            <h3
              class="text-lg font-semibold text-gray-900 mb-4 flex items-center"
            >
              <font-awesome-icon icon="fa-solid fa-file-upload" class="mr-2" />
              Submitted Documents
            </h3>
            ...
            <div class="space-y-4">
              ...
              <div
                ...
                class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white p-4 rounded-lg border-gray-200 border gap-3"
              >
                ...
                <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                  <button
                    title="View Resume"
                    @click="
                      viewFile(application.resume_path, 'resume', 'Resume')
                    "
                    class="btn btn-sm bg-accentColor border-none w-full sm:w-auto"
                  >
                    <font-awesome-icon icon="fa-solid fa-eye" class="" />
                  </button>
                  <button
              ...
              <div
                v-if="application.additional_documents_path"
                class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white p-4 rounded-lg border gap-3"
              >
                <div class="flex items-center">
                  <font-awesome-icon
                ...
                <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                  <button
                    ...
                    "
                    class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors text-sm w-full sm:w-auto"
                  >
                  ...
                  <button
                    ...
                    "
                    class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors text-sm w-full sm:w-auto"
                  >
                    <font-awesome-icon
                      icon="fa-solid fa-download"
                      class="mr-2"
      ...
      >
        ...
        >
          <div class="flex items-center gap-3">
            <font-awesome-icon
              :icon="
                isPdfFile(previewingDocument.url)
            ...
            <h3 class="text-lg font-semibold text-gray-900">
              {{
                previewingDocument.type === 'resume'
                  ? 'Resume/CV'
                  : 'Additional Documents'
              }}
            </h3>
            <span class="text-sm text-gray-500"
              >- {{ previewingDocument.name }}</span
            >
          </div>
          <button
        ...
        >
          ...
          <div class="flex gap-2">
            ...
            <button @click="closeDocumentPreview" class="btn btn-sm">
              Close
            </button>
          </div>
        </div>
...
<script setup>
  ...
  const loadApplicationDetails = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/job-applications/${props.applicationId}`,
        {
          headers: {
  ...
  const viewFile = (filePath, type, fileName) => {
    ...
    const backendUrl =
      apiConfig.baseURL.replace('/api', '') || 'http://localhost:5000';
    ...
    previewDocumentUrl.value =
      viewUrl + (viewUrl.toLowerCase().includes('.pdf') ? '#view=FitH' : '');
  ...
  const downloadFile = (filePath, type) => {
    ...
    const safeName = (application.value?.full_name || 'applicant').replace(
      /\s+/g,
      '_'
    );
  ...
  const getStatusColor = (status) => {
    const colors = {
      new: 'badge bg-success/10 text-green-800',
      reviewing: 'badge bg-info/10 text-blue-800',
      shortlisted: 'badge bg-purple-100 text-purple-800',
      rejected: 'badge bg-red-100 text-red-800',
      hired: 'badge bg-green-100 text-green-800',
    };
