<template>
  <dialog ref="modal" class="modal">
    <div class="modal-box max-w-[95vw] w-full max-h-[95vh] overflow-hidden flex flex-col">
      ...
      <div class="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 overflow-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="form-control">
            ...
            >
              ...
              <option value="promotional_banner">Promotional Banner Style</option>
            </select>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="form-control" v-if="showSubtitle">
            <label class="label">
        ...
        <div class="form-control" v-if="showDescription">
          <label class="label">
            <div class="flex items-center justify-between w-full">
              <div>
                <span class="label-text font-semibold">Description {{ textRequired ? '*' : '' }}</span>
                <span class="label-text-alt text-gray-500">(Supports HTML formatting)</span>
              </div>
        ...
        <div class="form-control" v-if="showImage">
          <div class="flex items-center justify-between mb-2">
            <label class="label py-0">
              <div class="flex items-center gap-2">
                <span class="label-text font-semibold">Images {{ imageRequired ? '*' : '' }}</span>
                <span class="label-text-alt text-gray-500">Enter URL or upload file</span>
              </div>
            </label>
          ...
          <div class="space-y-2">
            ...
            >
              <div class="flex gap-2 flex-1">
                ...
                />
                <input
                  :ref="el => { if (el) imageFileInputs[index] = el }"
                  type="file"
                  accept="image/*"
                  class="hidden"
                ...
                <button
                  ...
                  style="border-color: var(--color-primaryColor); color: var(--color-primaryColor);"
                >
                  <span v-if="uploadingImages[index]" class="loading loading-spinner loading-xs"></span>
                  <template v-else>
                    <FolderOpen :size="14" style="color: var(--color-primaryColor);" />
                  </template>
                </button>
                <button
          ...
          <!-- Image Previews -->
          <div v-if="imageList.some(img => img)" class="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
            <div
          ...
          <label class="label" v-if="imageUploadError">
            <span class="label-text-alt text-error">{{ imageUploadError }}</span>
          </label>
        </div>
        ...
        <div class="form-control" v-if="showVideo">
          <label class="label">
            <div class="flex items-center gap-2">
              <span class="label-text font-semibold">Video {{ videoRequired ? '*' : '' }}</span>
              <div class="flex gap-1">
                <button
                  type="button"
                  class="btn btn-sm btn-ghost btn-circle"
          ...
          <div class="flex gap-2">
            ...
            <button
              ...
              style="border-color: var(--color-primaryColor); color: var(--color-primaryColor);"
            >
              <span v-if="uploadingVideo" class="loading loading-spinner loading-sm"></span>
              <template v-else>
                <FolderOpen :size="16" style="color: var(--color-primaryColor);" />
                <span>Browse</span>
              </template>
            </button>
          </div>
          <label class="label" v-if="videoFile">
            <span class="label-text-alt flex items-center gap-1" style="color: var(--color-primaryColor);">
              <Paperclip :size="14" />
          ...
          <label class="label">
            <span class="label-text-alt flex items-center gap-1" style="color: var(--color-primaryColor);">
              <Lightbulb :size="14" />
              <span>Supported: YouTube, Vimeo, or direct video file uploads (MP4, WEBM, OGG, MOV)</span>
            </span>
          </label>
          <label class="label" v-if="videoUploadError">
            <span class="label-text-alt text-error">{{ videoUploadError }}</span>
          </label>
        </div>
        ...
        <div class="form-control" v-if="showPromoDetails">
          <div class="flex items-center justify-between mb-2">
            <label class="label py-0">
              <span class="label-text font-semibold">Promotion Details</span>
            </label>
            <select
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="form-control">
              ...
              <div class="flex items-center justify-between w-full">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <input
                    ...
                    style="--tglbg: var(--color-primaryColor-light); --handle-bg: var(--color-primaryColor);"
                  />
                </div>
              </div>
            </label>
            <input
              ...
              :placeholder="isNonExpiry ? 'Never expires' : 'Select expiry date'"
            />
          </div>
        </div>
        ...
        <div class="form-control">
          <label class="label cursor-pointer">
            ...
            <div class="modal-action sticky bottom-0 bg-white pt-4 border-t border-gray-200">
              <button
                type="button"
                class="btn btn-ghost"
                @click="closeModal"
        ...
        >
          <div class="sticky top-0 bg-white pb-2 mb-4 border-b border-gray-200 z-10">
            <div class="flex items-center justify-between">
              <div>
                <h4 class="font-bold text-lg text-gray-900">Preview</h4>
                <p class="text-xs text-gray-500">See how your announcement will appear</p>
              </div>
              <button
                type="button"
          ...
          <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div
              ...
              style="background: linear-gradient(to right, var(--color-primaryColor), var(--color-primaryColor));"
            >
              <div class="flex items-center space-x-2">
                <div class="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center">
                  <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H8a3.988 3.988 0 01-1.564-.317z" />
                  </svg>
                </div>
            ...
            <div class="p-4 space-y-4">
              ...
              <div
                v-if="formData.promo_details && showPromoDetails && formData.promo_position === 'above'"
              ...
              >
                <div class="flex items-start space-x-2">
                  ...
                  <div class="flex-1">
                    <h3 class="font-bold text-orange-800 text-xs mb-1">Promotion Details</h3>
                    <div class="text-orange-700 text-xs leading-relaxed whitespace-pre-line">
              ...
              <template v-for="(item, index) in (formData.content_order || [])" :key="`content-${index}-${item}-${formData.content_order?.join('-') || ''}`">
                <!-- Description Preview -->
                <div
                  v-if="item === 'description' && formData.description && showDescription"
                ...
                <div
                  v-if="item === 'images' && imageList.filter(img => img).length > 0 && showImage && formData.image_display_type === 'single'"
                ...
                <div
                  v-if="item === 'images' && imageList.filter(img => img).length > 1 && showImage && formData.image_display_type === 'carousel'"
                  class="rounded-lg overflow-hidden relative"
                >
                  <div class="relative w-full">
                    <div class="relative">
                      <img
                        v-for="(image, imgIndex) in imageList.filter(img => img)"
                        :key="imgIndex"
                        :src="image"
                        ...
                        @error="handleImageError"
                      />
                      <div v-if="imageList.filter(img => img).length > 1" class="absolute inset-0 flex items-center justify-between p-2 pointer-events-none">
                        <button
                          type="button"
                          class="btn btn-circle btn-sm bg-black/50 text-white border-none hover:bg-black/70 pointer-events-auto"
                          @click.stop="previewImageIndex = (previewImageIndex - 1 + imageList.filter(img => img).length) % imageList.filter(img => img).length"
                        >
                          <ChevronLeft :size="16" />
                        </button>
                        <button
                          type="button"
                          class="btn btn-circle btn-sm bg-black/50 text-white border-none hover:bg-black/70 pointer-events-auto"
                          @click.stop="previewImageIndex = (previewImageIndex + 1) % imageList.filter(img => img).length"
                        >
                          <ChevronRight :size="16" />
                        </button>
                      </div>
                      <div v-if="imageList.filter(img => img).length > 1" class="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex gap-1 pointer-events-none">
                        <div
                          v-for="(image, imgIndex) in imageList.filter(img => img)"
                          :key="imgIndex"
                          class="w-2 h-2 rounded-full pointer-events-auto"
                          :class="previewImageIndex === imgIndex ? 'bg-white' : 'bg-white/50'"
                        ></div>
                      </div>
                    </div>
                  </div>
                ...
                >
                  <div class="relative w-full" style="padding-bottom: 56.25%;">
                    <iframe
                      v-if="isYouTubeUrl(formData.video_url) || isVimeoUrl(formData.video_url)"
              ...
              <div
                v-if="formData.promo_details && showPromoDetails && formData.promo_position === 'below'"
              ...
              >
                <svg class="w-4 h-4" style="color: var(--color-primaryColor);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <!-- Empty State -->
              <div
                v-if="!formData.title && !formData.description && !formData.image_url && !formData.video_url"
...
<script setup>
  ...
  import { X, FolderOpen, Paperclip, Lightbulb, Plus, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Eye, EyeOff } from 'lucide-vue-next';
  ...
  const showDescription = computed(() => {
    ...
    return format === 'all' || format === 'text_image' || format === 'video_text' || format === 'text_only';
  });

  const showImage = computed(() => {
    const format = formData.value.content_format;
    return format === 'all' || format === 'text_image' || format === 'image_only';
  });

  const showVideo = computed(() => {
    const format = formData.value.content_format;
    return format === 'all' || format === 'video_text' || format === 'video_only';
  });
  ...
  const tinyMCEConfig = computed(() => ({
    ...
    height: 300,
    plugins: 'link lists',
    toolbar: 'undo redo | formatselect fontsize | bold italic underline strikethrough | forecolor backcolor | blockquote | alignleft aligncenter alignright alignjustify | bullist numlist | link | removeformat',
    toolbar_mode: 'wrap',
    font_size_formats: '8px 10px 12px 14px 16px 18px 20px 24px 28px 32px 36px 48px 60px 72px',
    ...
    valid_elements: 'p[style],br,strong/b[style],em/i[style],u[style],s/strike[style],span[style|class],div[style|class],blockquote[style],ul,ol,li,a[href|target|rel|style],h1[style],h2[style],h3[style],h4[style],h5[style],h6[style]',
    valid_styles: {
  ...
  const initializeContentOrder = () => {
    ...
    if (currentOrder.length === 0) {
      ...
      formData.value.content_order = defaultOrder.filter(item => available.includes(item));
      return;
    }
    
    // Filter current order to only include available items
    const filteredOrder = currentOrder.filter(item => available.includes(item));
  ...
  const moveContentUp = (item) => {
    if (!formData.value.content_order || formData.value.content_order.length === 0) {
  ...
  const loadAnnouncementData = () => {
    if (props.announcement) {
      ...
      if (props.announcement.images) {
        try {
          images = typeof props.announcement.images === 'string' 
    ...
    resetForm();
         ...
         const handleSubmit = () => {
           ...
           const submitData = {
             ...
             content_order: typeof formData.value.content_order === 'string' 
               ? formData.value.content_order 
               : JSON.stringify(formData.value.content_order || ['description', 'images', 'video']),
             // Clear valid_until if non-expiry is enabled
             valid_until: isNonExpiry.value ? null : (formData.value.valid_until || null),
           };
           
           console.log('Submitting announcement data:', JSON.stringify(submitData, null, 2));
...
<style scoped>
  ...
  .toggle {
    background-color: color-mix(in srgb, var(--color-primaryColor) 30%, transparent) !important;
