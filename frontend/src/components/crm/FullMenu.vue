  ...
  <div class="bg-gray-50 min-h-screen">
    ...
    <div class="container mx-auto px-4 sm:px-6 py-4 sm:py-8 lg:py-12 pt-20 sm:pt-24 lg:pt-32">
      ...
      <div class="lg:hidden mb-4">
        ...
        <!-- Horizontal Scrollable Category Filters (Mobile) -->
        <div class="overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4">
          <div class="flex gap-2 whitespace-nowrap">
            <button 
              ...
              :class="[
                'px-4 py-2 rounded-full font-medium transition-all duration-300 text-sm whitespace-nowrap flex-shrink-0',
      ...
      <!-- Desktop Title and Sidebar (Desktop Only) -->
      <div class="hidden lg:block mb-6 lg:mb-8 mt-4 sm:mt-6 lg:mt-8">
        <h1 class="text-3xl lg:text-5xl font-bold text-green-800">
          Menu
        </h1>
      </div>

      <!-- Category Filters and Menu Grid Container -->
      <div class="flex flex-col lg:flex-row gap-4 lg:gap-8">
        <!-- Side Category Filters (Desktop Only) -->
        <div class="hidden lg:block lg:w-64 lg:flex-shrink-0">
          <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-24 border border-gray-100">
            <h3 class="text-lg font-bold text-green-800 mb-4 flex items-center gap-2">
              <UtensilsCrossed :size="20" class="text-orange-500" />
              Categories
            </h3>
            <div class="space-y-2">
              <button 
                ...
                @click="selectedCategory = category"
                :class="[
                  'w-full text-left px-4 py-3 rounded-xl font-medium transition-all duration-300 flex items-center justify-between',
                  selectedCategory === category 
                    ? 'bg-orange-500 text-white shadow-md transform scale-[1.02]' 
                    : 'bg-gray-50 text-gray-700 hover:bg-gray-100 hover:text-green-700'
                ]"
              >
                <span>{{ category }}</span>
                <span 
        ...
        <div class="flex-1 min-w-0">
          <!-- Loading State -->
          <div v-if="isLoadingMenu" class="flex justify-center items-center py-8 sm:py-12">
            <div class="loading loading-spinner loading-lg text-green-600"></div>
          </div>

          <!-- Menu Grid -->
          <div v-if="!isLoadingMenu" class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-2.5 sm:gap-3 lg:gap-4 xl:gap-8">
        <div 
          v-for="item in paginatedMenuItems" 
          :key="item.name"
          @click="openItemModal(item)"
          class="group bg-white rounded-lg sm:rounded-xl lg:rounded-2xl shadow-md hover:shadow-xl lg:hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:-translate-y-1 lg:hover:-translate-y-3 relative border border-gray-100 hover:border-orange-200 cursor-pointer"
        >
          <!-- Promo Badge -->
          <div v-if="item.hasPromo" class="absolute top-2 right-2 sm:top-3 sm:right-3 z-20">
            <div
              class="bg-gradient-to-br from-red-500 to-orange-500 rounded-lg sm:rounded-xl px-2 py-1 sm:px-3 sm:py-2 shadow-xl backdrop-blur-sm border-2 border-white/30 relative overflow-hidden hover:scale-110 transition-all duration-300"
            >
              <div
              ...
              <div class="relative z-10 text-center">
                <div class="text-white text-xs sm:text-sm font-bold leading-none drop-shadow-md">
                  {{
                ...
                <div class="text-white/95 text-[8px] sm:text-[10px] font-semibold uppercase tracking-wider mt-0.5 sm:mt-1">
                  OFF
                </div>
              </div>
            </div>
          ...
          <div class="relative h-32 sm:h-40 lg:h-48 xl:h-56 overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200">
            <img 
              :src="item.image" 
              :alt="item.name" 
            ...
            <div v-if="item.category && item.category !== 'All'" class="absolute bottom-2 left-2 sm:bottom-3 sm:left-3">
              <span class="bg-white/90 backdrop-blur-sm text-green-700 text-[10px] sm:text-xs font-semibold px-2 sm:px-3 py-0.5 sm:py-1 rounded-full shadow-md">
                {{ item.category }}
              </span>
            </div>
          </div>
          ...
          <div class="p-3 sm:p-4 lg:p-5 xl:p-6">
            <!-- Item Name -->
            <h3 class="text-sm sm:text-base lg:text-lg xl:text-xl font-bold text-green-800 mb-1 sm:mb-2 group-hover:text-green-600 transition-colors duration-300 line-clamp-1">
              {{ item.name }}
            </h3>
            
            <!-- Description -->
            <p class="text-gray-600 text-[11px] sm:text-xs lg:text-sm mb-2 sm:mb-4 line-clamp-2 min-h-[2rem] sm:min-h-[2.5rem] leading-relaxed">
              {{ item.description }}
            </p>

            <!-- Price Section -->
            <div class="flex flex-col items-center justify-center pt-1.5 sm:pt-2 border-t border-gray-100">
              <!-- Original Price (crossed out if promo) -->
              <div v-if="item.hasPromo" class="mb-0.5 sm:mb-1">
                <span class="text-gray-400 text-[10px] sm:text-xs lg:text-sm line-through font-medium">
                  <font-awesome-icon icon="fa-solid fa-peso-sign" class="text-[8px] sm:text-xs" />
              ...
              <div class="flex items-center gap-0.5 sm:gap-1">
                <span
                  :class="[
                    'font-bold text-base sm:text-lg lg:text-xl xl:text-2xl',
                    item.hasPromo ? 'text-red-500' : 'text-green-700',
                  ]"
                >
                  <font-awesome-icon icon="fa-solid fa-peso-sign" class="text-xs sm:text-sm lg:text-base xl:text-lg" />
                  {{
              ...
              <div
                v-if="item.hasPromo"
                class="mt-1 sm:mt-2 px-2 sm:px-3 py-0.5 sm:py-1 bg-green-100 text-green-700 rounded-full text-[9px] sm:text-[10px] lg:text-xs font-semibold"
              >
                {{
                  item.promoInfo.discount_type === 'percentage'
          ...
          <!-- Hover Effect Border -->
          <div class="absolute inset-0 border-2 border-transparent group-hover:border-orange-300 rounded-2xl transition-all duration-500 pointer-events-none"></div>
        </div>
      </div>

          <!-- No Items Message -->
          <div v-if="!isLoadingMenu && filteredMenuItems.length === 0" class="text-center py-12">
            <p class="text-gray-500 text-lg">No items found in this category.</p>
          </div>

          <!-- Pagination Controls (Mobile Only) -->
          <div v-if="filteredMenuItems.length > itemsPerPage" class="flex lg:hidden justify-center items-center space-x-4 mt-8">
        <button 
          @click="currentPage--"
    ...
    >
      <div class="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl relative">
        ...
        <button
          ...
          class="absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-white/90 backdrop-blur-sm rounded-full p-3 hover:bg-white shadow-lg transition-all duration-300 hover:scale-110"
          aria-label="Previous item"
        >
          <font-awesome-icon icon="fa-solid fa-chevron-left" class="text-gray-700 text-lg" />
        </button>
        <button
          v-if="canNavigateNext"
          @click="navigateToNext"
          class="absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-white/90 backdrop-blur-sm rounded-full p-3 hover:bg-white shadow-lg transition-all duration-300 hover:scale-110"
          aria-label="Next item"
        >
          <font-awesome-icon icon="fa-solid fa-chevron-right" class="text-gray-700 text-lg" />
        </button>

        <!-- Item Indicator (Current Item / Total) -->
        <div class="absolute top-4 left-1/2 -translate-x-1/2 z-10 bg-black/50 backdrop-blur-sm rounded-full px-4 py-1.5">
          <span class="text-white text-sm font-medium">
            {{ currentItemIndex + 1 }} / {{ filteredMenuItems.length }}
          </span>
        </div>
        ...
        <div class="relative">
          ...
          <button
            ...
            class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-full p-2 hover:bg-white transition-colors z-10"
          >
            <font-awesome-icon icon="fa-solid fa-times" class="text-gray-700 text-lg" />
          </button>
          <!-- Category Badge -->
          <div v-if="selectedItem.category && selectedItem.category !== 'All'" class="absolute bottom-4 left-4">
            <span class="bg-white/90 backdrop-blur-sm text-green-700 text-sm font-semibold px-4 py-1.5 rounded-full shadow-md">
              {{ selectedItem.category }}
            </span>
          </div>
          <!-- Promo Badge -->
          <div v-if="selectedItem.hasPromo" class="absolute top-4 left-4">
            <div class="bg-gradient-to-br from-red-500 to-orange-500 rounded-xl px-3 py-2 shadow-xl backdrop-blur-sm border-2 border-white/30">
              <div class="text-white text-sm font-bold leading-none">
                {{
                  selectedItem.promoInfo.discount_type === 'percentage'
                    ? '-' + selectedItem.promoInfo.discount_percentage + '%'
                    : '-₱' + selectedItem.promoInfo.discount_amount
                }}
              </div>
              <div class="text-white/95 text-[10px] font-semibold uppercase tracking-wider mt-1">
                OFF
              </div>
            </div>
          </div>
        ...
        <div class="p-6">
          ...
          <!-- Price Section -->
          <div class="flex flex-col items-start mb-4 pb-4 border-b border-gray-200">
            <!-- Original Price (crossed out if promo) -->
            <div v-if="selectedItem.hasPromo" class="mb-1">
            ...
            <div class="flex items-center gap-1">
              <span
                :class="[
                  'font-bold text-3xl',
                  selectedItem.hasPromo ? 'text-red-500' : 'text-green-700',
                ]"
              >
                <font-awesome-icon icon="fa-solid fa-peso-sign" />
                {{
            ...
            <!-- Discount Badge -->
            <div
              v-if="selectedItem.hasPromo"
              class="mt-2 px-4 py-1.5 bg-green-100 text-green-700 rounded-full text-sm font-semibold"
            >
              {{
                selectedItem.promoInfo.discount_type === 'percentage'
                  ? selectedItem.promoInfo.discount_percentage + '% OFF'
                  : '₱' + selectedItem.promoInfo.discount_amount + ' OFF'
              }}
            </div>
          </div>
...
import { UtensilsCrossed, ChefHat, Sparkles } from 'lucide-vue-next';
...
const fetchMenuItems = async () => {
  try {
    ...
    const response = await menuService.getPublicMenuItems({ limit: 100 }); // Get all items
...
const currentItemIndex = computed(() => {
  ...
  return filteredMenuItems.value.findIndex(item => item.name === selectedItem.value.name);
});
...
const canNavigateNext = computed(() => {
  return currentItemIndex.value < filteredMenuItems.value.length - 1;
});
...
const navigateToNext = () => {
  if (canNavigateNext.value) {
    ...
    selectedItem.value = filteredMenuItems.value[nextIndex];
  }
};
...
const navigateToPrev = () => {
  if (canNavigatePrev.value) {
    ...
    selectedItem.value = filteredMenuItems.value[prevIndex];
  }
};
...
const handleTouchMove = (e) => {
  // Allow scrolling, don't prevent default to allow natural scrolling
};
...
const handleTouchEnd = (e) => {
  ...
  if (Math.abs(swipeDistanceX) > swipeDistanceY && Math.abs(swipeDistanceX) > minSwipeDistance) {
    // Swipe left (next item) - swipe distance > 50px
    if (swipeDistanceX > minSwipeDistance && canNavigateNext.value) {
      navigateToNext();
    }
    // Swipe right (previous item) - swipe distance < -50px
    else if (swipeDistanceX < -minSwipeDistance && canNavigatePrev.value) {
      navigateToPrev();
    }
  }
};
...
const handleKeyDown = (e) => {
  ...
  if (e.key === 'ArrowLeft' && canNavigatePrev.value) {
    navigateToPrev();
...
/* Shimmer Animation for Promo Badge */
@keyframes shimmer {
  0% {
...
/* Horizontal scroll for mobile category filters */
.scrollbar-hide {
