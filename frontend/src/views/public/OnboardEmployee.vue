...
const verifyToken = async () => {
  try {
    ...
    if (!token) {
      errorMessage.value = 'No onboarding token provided. Please use the link from your hire email.';
      return;
    }

    onboardingToken.value = token;

    const response = await fetch(`${apiConfig.baseURL}/onboarding/verify/${token}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok || !data.success) {
      errorMessage.value = data.message || 'Invalid or expired onboarding link';
      return;
    }
    ...
    if (data.isResubmission) {
      ...
      employeeForm.value.philhealth_number = data.data.philhealth_number || '';
      employeeForm.value.emergency_contact_name = data.data.emergency_contact_name || '';
      employeeForm.value.emergency_relationship = data.data.emergency_relationship || '';
      employeeForm.value.emergency_contact_number = data.data.emergency_contact_number || '';
      employeeForm.value.emergency_contact_address = data.data.emergency_contact_address || '';
    }
    ...
    errorMessage.value = 'Failed to verify onboarding link. Please contact HR.';
  } finally {
    loading.value = false;
  }
};
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.success && data.data) {
        ...
        console.warn('Roles API returned no data, checking onboarding data...');
      }
    }
  } catch (error) {
...
const baseSalary = computed(() => {
  ...
  const hoursPerDay = employeeForm.value.employee_type === 'Full-time' ? 8 : 4;
...
const getPdfPreviewUrl = computed(() => {
  if (previewModal.value.file && previewModal.value.file.type === 'application/pdf') {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!response.ok || !data.success) {
      ...
      const errorMsg = data.error || data.message || 'Failed to submit onboarding form';
    ...
    if (photoFile.value && data.data?.id) {
      try {
        ...
        const photoResponse = await fetch(`${apiConfig.baseURL}/employees/${data.data.id}/photo`, {
          method: 'POST',
          body: formData,
        });
        
        if (!photoResponse.ok) {
          console.warn('Photo upload endpoint not available or failed:', photoResponse.status);
        }
      } catch (photoError) {
        console.warn('Photo upload failed, but employee was created:', photoError);
    ...
    if (data.data?.id) {
      const documentsToUpload = [
        ...
        { file: medicalCertFile.value, name: 'medical_cert', label: 'Medical Certificate' },
      ...
      for (const doc of documentsToUpload) {
        if (doc.file) {
          try {
            ...
            const uploadResponse = await fetch(`${apiConfig.baseURL}/employees/${data.data.id}/documents`, {
              method: 'POST',
              body: formData,
            });

            console.log(`${doc.label} upload response status:`, uploadResponse.status);
            ...
            if (!uploadResponse.ok) {
              ...
              uploadErrors.push(`${doc.label}: ${errorData.message || 'Upload failed'}`);
            } else {
              const uploadResult = await uploadResponse.json();
              console.log(`${doc.label} uploaded successfully:`, uploadResult);
              uploadedCount++;
            }
          } catch (docError) {
            console.error(`${doc.label} upload error:`, docError);
            uploadErrors.push(`${doc.label}: ${docError.message || 'Network error'}`);
          }
        } else {
      ...
      console.log(`Document upload complete: ${uploadedCount}/${documentsToUpload.length} uploaded`);
    } else {
      console.error('Employee ID not returned from server, cannot upload documents');
    }
...
<template>
  <div class="min-h-screen py-8 px-4 onboarding-container" :style="containerStyle">
      ...
      <div class="text-center mb-8">
        <div class="flex justify-center mb-5">
          <div class="relative">
            ...
            <div class="w-32 h-32 rounded-full bg-white flex items-center justify-center shadow-2xl ring-4 ring-white/20">
              <div class="w-28 h-28 rounded-full border-4 border-primaryColor flex items-center justify-center bg-white">
                <img :src="Logo" alt="Countryside Steak House" class="h-20 w-20 object-contain" />
              </div>
            </div>
          </div>
        </div>
        ...
        <p class="text-lg text-green-100 font-medium mb-1">
          Welcome to Countryside Steak House
        </p>
        <p class="text-sm text-green-50/80">
          Complete the required information to get hired.
        </p>
      </div>

      <!-- Loading State -->
      <div v-if="loading" class="bg-white rounded-2xl shadow-xl border border-gray-100">
        <div class="text-center py-16 px-6">
          <span class="loading loading-spinner loading-lg text-primaryColor"></span>
          <p class="mt-6 text-gray-700 font-medium text-lg">Verifying your onboarding link...</p>
        </div>
      </div>

      <!-- Error State -->
      <div v-else-if="errorMessage" class="bg-white rounded-2xl shadow-xl border border-red-200">
        <div class="p-6">
      ...
      <div v-else class="space-y-5">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-5">
          <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-base font-bold text-primaryColor">{{ completionPercentage }}%</span>
          </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
          ...
          <div class="flex justify-between items-start gap-4">
            ...
            >
              <div
                class="w-16 h-16 rounded-full flex items-center justify-center mb-3 transition-all duration-300 shadow-md"
                :class="{
                  'bg-gradient-to-br from-primaryColor to-green-600 text-white shadow-lg shadow-primaryColor/30 scale-105': index <= currentStep,
                  'bg-gray-200 text-gray-400 group-hover:bg-gray-300': index > currentStep,
                  'ring-4 ring-primaryColor/30 ring-offset-2 ring-offset-white scale-110': index === currentStep,
                }"
              >
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          <div class="p-8">
            ...
            <div class="mb-8 pb-6 border-b border-gray-200">
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex-1">
                  ...
                  />
                  <p v-if="!photoFile" class="text-xs text-gray-500 mt-2">No file selected</p>
                </div>
              </div>
            </div>
            ...
            <div class="mb-6">
              ...
              >
                <component :is="currentStepData.icon" class="w-6 h-6 text-primaryColor" />
            ...
            <div v-if="currentStep === 0" class="space-y-6">
              ...
              <div
                class="grid grid-cols-1 lg:grid-cols-3 gap-4 bg-gradient-to-br from-gray-50 to-gray-100 p-6 rounded-2xl shadow-inner border border-gray-200"
              >
                <!-- First Name -->
                ...
                <div class="form-control">
                  <label class="label">
                    <span class="label-text"
                      ...
                      <span class="text-gray-500 text-xs">(Auto-calculated)</span></span
                    >
                  </label>
                  <input
              ...
              <div class="mt-6 space-y-4">
                ...
                <p class="text-sm text-gray-600 mb-4">
                  Please attach the following required documents. All documents must be clear and readable.
                </p>
                ...
                <div class="mb-4">
                  ...
                  <div class="flex gap-2">
                    ...
                    <button
                      v-if="validIdFile"
                      type="button"
                      @click="showPreview('Valid ID', validIdFile, validIdPreview)"
                      @mouseenter="showHoverPreview($event, 'Valid ID', validIdFile, validIdPreview)"
                      @mouseleave="hideHoverPreview"
                      class="btn btn-sm btn-outline btn-primary relative"
                    >
                  ...
                  <p class="text-xs text-gray-500 mt-1">
                    Accepted formats: PDF, JPG, PNG (Max 5MB) | Use camera or browse
                  </p>
                  <p 
                    v-if="validIdFile" 
                    ...
                    @mouseleave="hideHoverPreview"
                  >
                    ✓ Selected: {{ validIdFile.name }} ({{ (validIdFile.size / 1024 / 1024).toFixed(2) }} MB)
                  </p>
                </div>
                ...
                <div class="mb-4">
                  <label class="block text-sm font-semibold text-gray-700 mb-2">
                    Medical Certificate or Pre-employment Medical Exam <span class="text-red-500">*</span>
                  </label>
                  <div class="flex gap-2">
                    <input
                      type="file"
                    ...
                    <button
                      ...
                      type="button"
                      @click="showPreview('Medical Certificate', medicalCertFile, medicalCertPreview)"
                      @mouseenter="showHoverPreview($event, 'Medical Certificate', medicalCertFile, medicalCertPreview)"
                      @mouseleave="hideHoverPreview"
                      class="btn btn-sm btn-outline btn-primary relative"
                    >
                      <Info class="w-4 h-4 mr-1" />
                  ...
                  <p 
                    ...
                    @mouseleave="hideHoverPreview"
                  >
                    ✓ Selected: {{ medicalCertFile.name }} ({{ (medicalCertFile.size / 1024 / 1024).toFixed(2) }} MB)
                  </p>
                </div>
                ...
                <div class="mb-4">
                  <label class="block text-sm font-semibold text-gray-700 mb-2">
                    Barangay Clearance, Police Clearance or NBI Clearance <span class="text-red-500">*</span>
                  </label>
                  <div class="flex gap-2">
                    <input
                      type="file"
                    ...
                    <button
                      v-if="clearanceFile"
                      type="button"
                      @click="showPreview('Clearance', clearanceFile, clearancePreview)"
                      @mouseenter="showHoverPreview($event, 'Clearance', clearanceFile, clearancePreview)"
                      @mouseleave="hideHoverPreview"
                      class="btn btn-sm btn-outline btn-primary relative"
                    >
                      <Info class="w-4 h-4 mr-1" />
                  ...
                  <p 
                    ...
                    @mouseleave="hideHoverPreview"
                  >
                    ✓ Selected: {{ clearanceFile.name }} ({{ (clearanceFile.size / 1024 / 1024).toFixed(2) }} MB)
                  </p>
                </div>

                <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 mt-4">
                  <p class="text-xs text-gray-700">
                    <strong>Note:</strong> All documents will be securely stored and used for employment verification purposes only.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div v-if="currentStep === 1" class="space-y-6">
              ...
              <div class="bg-gradient-to-br from-primaryColor/10 to-green-50 p-6 rounded-2xl shadow-lg border-2 border-primaryColor/30">
                <h3 class="font-bold text-xl mb-6 text-gray-800 flex items-center">
                  <DollarSign class="w-6 h-6 mr-2 text-primaryColor" />
                  Base Salary Information
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div class="bg-white p-4 rounded-xl shadow-md border border-primaryColor/20">
                    <label class="text-sm font-medium text-gray-600 mb-2 block">Hourly Rate</label>
                    <div class="text-3xl font-bold text-primaryColor">
                      {{ baseSalary?.formatted?.hourly || '₱0.00' }}
                    </div>
                  </div>
                  <div class="bg-white p-4 rounded-xl shadow-md border border-primaryColor/20">
                    <label class="text-sm font-medium text-gray-600 mb-2 block">Daily Rate</label>
                    <div class="text-3xl font-bold text-primaryColor">
                      {{ baseSalary?.formatted?.daily || '₱0.00' }}
                    </div>
                    <p class="text-xs text-gray-600 mt-2 font-medium">
                      {{ employeeForm.employee_type === 'Full-time' ? '8' : '4' }} hours per day
                    </p>
                  </div>
                  <div class="bg-white p-4 rounded-xl shadow-md border border-primaryColor/20">
                    <label class="text-sm font-medium text-gray-600 mb-2 block">Monthly Rate</label>
                    <div class="text-3xl font-bold text-primaryColor">
                      {{ baseSalary?.formatted?.monthly || '₱0.00' }}
                    </div>
                    <p class="text-xs text-gray-600 mt-2 font-medium">26 working days</p>
                  </div>
                </div>
                <div
              ...
              <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
                ...
                <div class="form-control">
                  <label class="label">
                    <span class="label-text"
                      >PhilHealth Number <span class="text-red-500">*</span></span
                    >
                  </label>
                  <input
              ...
              <!-- Info Alert -->
              <div class="alert alert-success shadow-lg border-2 border-green-200">
            ...
            <div v-if="currentStep === 2" class="space-y-6">
              <div class="bg-blue-50 p-4 rounded-xl border border-blue-200 mb-4">
                <p class="text-sm text-gray-700">
                  ...
                  Please provide accurate emergency contact information. This will be used in case of emergencies.
                </p>
              </div>
              ...
              <div class="form-control">
                <label class="label">
                  <span class="label-text"
                    >Emergency Contact Name <span class="text-red-500">*</span></span
                  >
                </label>
                <input
              ...
              <div class="form-control">
                <label class="label">
                  <span class="label-text"
                    >Emergency Contact Number <span class="text-red-500">*</span></span
                  >
                </label>
                <input
                  ...
                  maxlength="11"
                  class="input input-bordered w-full"
                  @input="handlePHPhoneInput($event.target.value, 'emergency_contact_number')"
                />
              </div>
              ...
              <div class="form-control">
                ...
                <input
                  ...
                  maxlength="11"
                  class="input input-bordered w-full"
                  @input="handlePHPhoneInput($event.target.value, 'alternate_contact_number')"
                />
              </div>
              ...
              <div class="form-control">
                <label class="label">
                  <span class="label-text"
                    >Emergency Contact Address <span class="text-red-500">*</span></span
                  >
                </label>
                <textarea
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mt-10 pt-6 border-t border-gray-200 gap-4">
              <div class="text-xs font-medium text-gray-600 bg-gray-50 px-4 py-2 rounded-full">
                Form Completion: <span class="text-primaryColor font-bold">{{ completionPercentage }}%</span>
              </div>
              <div class="flex gap-3">
                <button
                  v-if="currentStep > 0"
                  @click="prevStep"
                  class="btn btn-outline hover:bg-gray-50 font-medium border-gray-300 text-gray-700"
                >
                ...
                <button
                  ...
                  @click="nextStep"
                  class="btn bg-primaryColor hover:bg-green-700 text-white font-semibold shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed border-0"
                ...
                <button
                  v-else
                  @click="handleSubmit"
                  class="btn bg-green-600 hover:bg-green-700 text-white font-semibold shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed border-0"
      ...
      <div class="modal-box max-w-4xl w-full">
        ...
        <div class="mb-4">
          <p class="text-sm text-gray-600">
            ...
            <strong>File Size:</strong> {{ previewModal.file ? (previewModal.file.size / 1024 / 1024).toFixed(2) : '0' }} MB<br>
        ...
        <div class="w-full max-h-[70vh] overflow-auto">
          <div v-if="previewModal.preview && previewModal.file?.type?.startsWith('image/')" class="flex justify-center">
            <img
          ...
          <div v-else-if="previewModal.file?.type === 'application/pdf'" class="w-full">
    ...
    <dialog id="completion_notice_modal" class="modal" :open="showCompletionNotice">
      <div class="modal-box max-w-xl w-full text-center">
        ...
        <h3 class="text-2xl font-bold text-primaryColor mb-2">Onboarding Submitted</h3>
        <p class="text-gray-700 leading-relaxed">
          This form will be reviewed by HR. Please wait for approval. If you have concerns,
          please email <a href="mailto:hr@countryside-steakhouse.site" class="text-primaryColor underline">hr@countryside-steakhouse.site</a>.
        </p>
        <div class="modal-action justify-center mt-6">
          <button
            class="btn btn-primary"
            @click="closeCompletionNotice"
          >
            OK
          </button>
        </div>
    ...
    <Transition name="fade">
      <div
        ...
        @mouseenter="() => { if (hoverPreviewTimeout) clearTimeout(hoverPreviewTimeout); hoverPreviewTimeout = null; }"
        @mouseleave="hideHoverPreview"
      >
      <!-- Preview Image -->
      <div v-if="hoverPreview.preview && hoverPreview.file?.type?.startsWith('image/')" class="w-full">
        <img
      ...
      <!-- PDF Preview -->
      <div v-else-if="hoverPreview.file?.type === 'application/pdf'" class="p-4 bg-gray-50">
      ...
      <div class="p-3 bg-white border-t border-gray-200">
        <p class="text-xs font-semibold text-gray-800 mb-1">{{ hoverPreview.title }}</p>
        <p class="text-xs text-gray-600 truncate">{{ hoverPreview.file?.name }}</p>
        <p class="text-xs text-gray-500 mt-1">
          {{ hoverPreview.file ? (hoverPreview.file.size / 1024).toFixed(1) : '0' }} KB
        </p>
      </div>
    </div>
    </Transition>
