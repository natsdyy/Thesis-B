  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <!-- Header -->
    <header :class="['text-white shadow-lg fixed top-0 left-0 right-0 z-50 transition-all duration-300', 
      isScrolled ? 'bg-green-800' : 'bg-transparent']">
      <div class="container mx-auto px-6 py-4">
        <div class="flex items-center justify-between">
          <!-- Logo and Brand -->
          <div class="flex items-center space-x-4 cursor-pointer group" @click="scrollToHome" title="Click to go to top">
          ...
          <nav class="hidden md:flex items-center space-x-10">
            <a @click="scrollToHome" class="relative group py-2 px-1 text-white hover:text-orange-300 transition-all duration-300 font-medium cursor-pointer">
            ...
            <router-link to="/stores" class="relative group py-2 px-1 text-white hover:text-orange-300 transition-all duration-300 font-medium">
    ...
    <!-- Hero Section -->
    <section id="home" class="relative min-h-screen flex items-center pt-20 hero-section overflow-hidden">
      ...
      <div class="absolute inset-0 bg-gradient-to-br from-black/40 via-black/20 to-green-900/30 z-20"></div>
      ...
      <button 
        @click="previousHero"
        class="absolute left-6 top-1/2 transform -translate-y-1/2 z-20 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full p-3 text-white hover:bg-white/20 transition-all duration-300 group"
      >
      ...
      <button 
        @click="nextHero"
        class="absolute right-6 top-1/2 transform -translate-y-1/2 z-20 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full p-3 text-white hover:bg-white/20 transition-all duration-300 group"
      >
        <svg class="w-6 h-6 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="relative z-30 container mx-auto px-6">
        <div class="grid lg:grid-cols-2 gap-12 items-center min-h-[80vh]">
          ...
          <div class="text-left space-y-8">
            <!-- Brand Badge -->
            <div class="inline-flex items-center bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-6 py-3 text-white mb-6">
              <span class="text-sm font-medium">🏆 Since 1984</span>
            </div>
            ...
            <!-- Subtitle -->
            <p class="text-xl lg:text-2xl text-white/90 max-w-lg leading-relaxed">
              Sabi nga nila, sa unang tikim pa lang, alam mong babalik-balikan mo! Experience the authentic taste that made us famous.
            </p>
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-4">
              <router-link 
                to="/menu" 
                class="group bg-orange-500 hover:bg-orange-600 text-white px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center"
              >
                <span>View Menu</span>
              ...
              <router-link to="/stores" class="group bg-transparent border-2 border-white text-white hover:bg-white hover:text-gray-800 px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center">
           ...
           <div class="text-center lg:text-right space-y-8 lg:space-y-10 lg:flex lg:flex-col lg:items-end">
             <!-- Reels Carousel Container -->
             <div class="relative">
               
               
                               <!-- Enhanced Video Display Area - Wider to Show All Content -->
                <div class="relative w-80 h-[32rem] lg:w-[32rem] lg:h-[40rem] mx-auto lg:mx-0 lg:ml-auto rounded-5xl shadow-2xl overflow-hidden video-player-container">
                 <!-- Current Video -->
                 <video 
                   :src="currentReel"
                 ...
                 >
                   <svg v-if="isHeroVideoPlaying" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button 
                    @click="previousReel"
                    class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black/70 hover:bg-black/90 text-white p-4 rounded-full transition-all duration-300 hover:scale-110 backdrop-blur-sm z-20">
                  ...
                  <button 
                    @click="nextReel"
                    class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black/70 hover:bg-black/90 text-white p-4 rounded-full transition-all duration-300 hover:scale-110 backdrop-blur-sm z-20">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                    </svg>
                  </button>
                ...
                <div class="text-center mt-6 space-y-3">
                  <div class="flex items-center justify-center space-x-3">
                    <span class="text-base font-semibold text-white">{{ currentReelIndex + 1 }} of {{ reels.length }}</span>
                    <span class="text-sm text-white/70">•</span>
                    <span class="text-sm text-orange-400 font-bold">{{ getReelTitle(currentReelIndex) }}</span>
                  </div>
                  <p class="text-sm text-white/90 font-medium">{{ getReelDescription(currentReelIndex) }}</p>
                  ...
                  <div class="flex items-center justify-center space-x-2 mt-2">
                    <div 
                      :class="[
                        ...
                        isReelsAutoScrollActive ? 'bg-orange-400 animate-pulse' : 'bg-gray-400'
                      ]"
                    ></div>
                    <span class="text-xs text-white/70">
                      {{ isReelsAutoScrollActive ? 'Auto-scroll active' : 'Auto-scroll paused' }}
                    </span>
                  </div>
                </div>
             </div>
      ...
      <div class="container mx-auto px-6">
        ...
        <div class="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div class="relative">
            ...
            <div class="absolute -bottom-6 -right-6 bg-orange-500 text-white px-6 py-3 rounded-full shadow-lg">
              <div class="text-center">
          ...
          <div class="space-y-8">
            <!-- Main Heading -->
            <div>
              <h2 class="text-4xl lg:text-5xl font-bold text-green-800 mb-6 leading-tight">
                Serving Sizzling Steaks Since 1984! 🔥🍴
              </h2>
            ...
            <div class="space-y-6 text-lg text-gray-700 leading-relaxed">
              <p>
                Welcome to <span class="font-semibold text-green-700">Countryside Steakhouse</span>, where tradition meets innovation in every sizzling plate. For over four decades, we've been crafting the perfect steak experience, combining time-honored recipes with modern culinary techniques.
              </p>
              
              <p>
                Our commitment to quality starts with the finest ingredients, sourced locally and prepared with passion. Every dish tells a story of dedication, from our signature sizzling steaks to our beloved silog breakfasts that have become a morning ritual for countless families.
              </p>
              
              <p>
                <span class="font-semibold text-orange-600">"Silog is the best in Countryside"</span> - this isn't just a saying, it's a promise we've kept for generations. Our silog dishes, featuring perfectly cooked rice, fresh eggs, and premium meats, have earned us the reputation as Cavite's premier breakfast destination.
              </p>
            </div>
            
            <!-- Branches Section -->
            <div class="bg-white rounded-xl p-6 shadow-lg border-l-4 border-green-600">
              <h3 class="text-xl font-bold text-green-800 mb-4">🌿 Growing Across Cavite</h3>
              <p class="text-gray-600 mb-4">We're proud to serve our community across multiple locations:</p>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
              <router-link 
                to="/menu" 
                class="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg font-semibold transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center"
              >
              ...
              <router-link 
                to="/stores" 
                class="bg-orange-500 hover:bg-orange-600 text-white px-8 py-3 rounded-lg font-semibold transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center"
              >
      ...
      <div class="container mx-auto px-6">
        <div class="text-center mb-16">
          <div class="flex items-center justify-center mb-4">
            <div class="w-16 h-0.5 bg-orange-400"></div>
            <h2 class="text-4xl font-bold text-white mx-4">Order for Delivery</h2>
            <div class="w-16 h-0.5 bg-orange-400"></div>
          </div>
          <p class="text-xl text-green-100 max-w-2xl mx-auto">
            Craving our delicious steaks? Order now through your favorite delivery platforms!
          </p>
        </div>
        ...
        <div class="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div class="space-y-8">
            <div class="text-center lg:text-left">
              <h3 class="text-2xl font-bold text-white mb-4">Choose Your Delivery Platform</h3>
              <p class="text-green-100">We partner with the best delivery services to bring Countryside to your doorstep</p>
            </div>
            
            <!-- Delivery Cards -->
            <div class="space-y-6">
              <!-- Foodpanda Card -->
              <div class="bg-orange-500 rounded-2xl p-6 text-white text-center hover:transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl">
                <div class="w-20 h-20 bg-white bg-opacity-20 rounded-full mx-auto mb-4 flex items-center justify-center">
                  <img src="/src/assets/crm/Foodpanda.png" alt="Foodpanda Logo" class="w-14 h-14 object-contain" />
                </div>
                <h3 class="text-2xl font-bold mb-3">Foodpanda</h3>
                <p class="text-base mb-4 opacity-90">Order your favorite Countryside dishes through Foodpanda</p>
                <button @click="openFoodpandaModal" class="bg-white text-orange-500 px-6 py-2 rounded-lg font-semibold hover:bg-gray-100 transition-colors w-full">
                  Order on Foodpanda
                </button>
              </div>

              <!-- Grabfood Card -->
              <div class="bg-green-600 rounded-2xl p-6 text-white text-center hover:transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl">
                <div class="w-20 h-20 bg-white bg-opacity-20 rounded-full mx-auto mb-4 flex items-center justify-center">
                  <img src="/src/assets/crm/GrabFood.png" alt="GrabFood Logo" class="w-14 h-14 object-contain" />
                </div>
                <h3 class="text-2xl font-bold mb-3">Grabfood</h3>
                <p class="text-base mb-4 opacity-90">Get your Countryside favorites delivered by Grab</p>
                <button @click="openGrabfoodModal" class="bg-white text-green-600 px-6 py-2 rounded-lg hover:bg-gray-100 transition-colors w-full">
                  Order on Grabfood
                </button>
              </div>
            </div>
          ...
          <div class="flex justify-center lg:justify-end">
            <div class="max-w-lg w-full">
              <div class="relative rounded-2xl overflow-hidden shadow-2xl">
                ...
                >
                  <svg v-if="isDeliveryVideoPlaying" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                  </svg>
                  <svg v-else class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h1m4 0h1m-6 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
              ...
              <!-- Video Caption -->
              <p class="text-sm text-gray-500 mt-3 italic text-center lg:text-right">
                Experience the convenience of food delivery with Countryside Steakhouse
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="mt-12 text-center">
          <div class="bg-gray-50 rounded-2xl p-6 max-w-2xl mx-auto">
            <div class="flex items-center justify-center mb-3">
              ...
              <h4 class="text-xl font-semibold text-green-800 mx-3">Delivery Information</h4>
              <div class="w-8 h-0.5 bg-green-600"></div>
            </div>
    ...
    <section id="stores" class="py-20 bg-white">
      <div class="container mx-auto px-6">
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <!-- Burol Main Branch -->
          <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 card-hover">
            ...
            <div class="p-6">
              <h3 class="text-xl font-bold text-green-800 mb-2">Countryside Burol Main Branch</h3>
          ...
          <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 card-hover">
            ...
            <div class="p-6">
              <h3 class="text-xl font-bold text-green-800 mb-2">Countryside Malihan Main Branch</h3>
              <p class="text-gray-600 text-sm mb-3">P&N COUNTRYSIDE STEAKHOUSE</p>
              <div class="flex items-center text-sm text-gray-600 mb-3">
                <span class="mr-2">📍</span>
          ...
          <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 card-hover">
            ...
            <div class="p-6">
              <h3 class="text-xl font-bold text-green-800 mb-2">Countryside Imus Branch</h3>
              <p class="text-gray-600 text-sm mb-3">COUNTRYSIDE STEAKHOUSE</p>
              <div class="flex items-center text-sm text-gray-600 mb-3">
                <span class="mr-2">📍</span>
    ...
    <section id="menu-teaser" class="py-20 bg-green-800">
      <div class="container mx-auto px-6">
        <div class="text-center mb-16">
          <div class="flex items-center justify-center mb-6">
            <div class="w-20 h-0.5 bg-orange-400"></div>
            <h2 class="text-4xl font-bold text-white mx-6">Our Signature Dishes</h2>
            <div class="w-20 h-0.5 bg-orange-400"></div>
          </div>
          <p class="text-xl text-white/90 max-w-2xl mx-auto">
            A glimpse of our most beloved dishes that keep our customers coming back
          </p>
        </div>
        ...
        <div class="text-center mt-12">
          <router-link 
            to="/menu" 
            class="bg-orange-500 text-white px-10 py-4 rounded-xl font-semibold text-lg hover:bg-orange-600 transition-colors inline-flex items-center group"
          >
            View Full Menu
      ...
      <div class="container mx-auto px-6">
        ...
        <div class="grid lg:grid-cols-2 gap-12">
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center">
                ...
                <div>
                  ...
                  <div class="text-gray-600">countryside_steakhouse@yahoo.com.ph</div>
                </div>
              </div>
              <div class="flex items-center">
                <div class="w-12 h-12 bg-green-600 rounded-full flex items-center justify-center mr-4">
          ...
          <div class="bg-white rounded-2xl p-8 shadow-lg">
            <h3 class="text-2xl font-bold text-green-800 mb-6">Send us a Message</h3>
            <form @submit.prevent="submitFeedback" class="space-y-4">
              ...
              <div>
                ...
                <div class="flex items-center space-x-1 star-rating">
                  <button 
                    ...
                    :class="[
                      ...
                      feedbackForm.rating >= star || hoveredRating >= star ? 'text-yellow-400' : 'text-gray-300'
                    ]"
                  >
                    ...
                    <svg 
                      ...
                      :class="[
                        feedbackForm.rating >= star || hoveredRating >= star ? 'scale-110 drop-shadow-lg' : 'scale-100'
                      ]"
                      fill="currentColor" 
                      viewBox="0 0 24 24"
                    >
                ...
                <!-- Rating description -->
                <div v-if="feedbackForm.rating > 0" class="mt-2 text-xs text-gray-500 animate-fade-in">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-3">
                  📸 Share Your Food Experience <span class="text-red-500">*</span>
                </label>
                <div class="space-y-3">
                  <!-- File Input -->
                  <div class="relative">
              ...
              <button 
                ...
                class="w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transform hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-xl"
              >
            ...
            <div v-if="feedbackMessage.show" :class="[
              'mt-4 p-4 rounded-lg',
              feedbackMessage.type === 'success' ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200'
            ]">
    ...
    <footer class="bg-orange-500 text-white py-12">
      <div class="container mx-auto px-6">
          <div class="flex flex-col md:flex-row justify-between items-center">
            ...
            <a href="#" class="hover:text-green-200 transition-colors">PRIVACY POLICY</a>
            <a href="#" class="hover:text-green-200 transition-colors">CONTACT US</a>
            </div>
            <div class="text-center md:text-right">
              <p class="text-sm">© 2025 COUNTRYSIDE STEAKHOUSE. All rights reserved.</p>
          </div>
        </div>
      </div>
    </footer>
...
const previousReel = () => {
  currentReelIndex.value = currentReelIndex.value === 0 
    ? reels.length - 1 
...
const toggleDeliveryVideo = () => {
  const deliveryVideo = document.querySelector('#deliveryVideo') || document.querySelector('video[ref="deliveryVideo"]');
  if (deliveryVideo) {
...
const submitFeedback = async () => {
  ...
  if (!feedbackForm.value.image || !feedbackForm.value.image.file) {
    showFeedbackMessage('error', 'Please upload an image of your food experience.');
    return;
  }
  ...
  try {
    ...
    if (response.success) {
      ...
      showFeedbackMessage('error', response.message || 'Failed to submit feedback.');
    }
  } catch (error) {
...
const handleImageError = (index) => {
  console.error(`Image failed to load at index ${index}:`, heroImages[index].src);
